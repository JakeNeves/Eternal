...
{
    public class CosmicStarstryderTreadsPlayer : ModPlayer
    {
		...
		public override void ResetEffects()
		{
			...
			for (int i = 3; i < 8 + Player.extraAccessorySlots; i++)
			{
				...
				if (item.type == ModContent.ItemType<CosmicStarstryderTreads>())
					dashAccessoryEquipped = true;
				else if (item.type == ItemID.EoCShield || item.type == ItemID.MasterNinjaGear || item.type == ItemID.FrogGear || item.type == ItemID.Tabi)
					return;
			}
