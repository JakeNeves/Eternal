...
namespace Eternal.NPCs.Boss.BionicBosses
{
    ...
    public class Proton : ModNPC
    {
        ...
        {
            DisplayName.SetDefault("XE-76 Proton");
            ...
            else
            {
                if (attackTimer == 150 || attackTimer == 155 || attackTimer == 160 || attackTimer == 165 || attackTimer == 350 || attackTimer == 355 || attackTimer == 360 || attackTimer == 365)
                {
                    ...
                    for (int i = 0; i < amountOfProjectiles; ++i)
                    {
                        ...
                        int damage = Main.expertMode ? 20 : 30;
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                            Projectile.NewProjectile(npc.Center.X, npc.Center.Y, direction.X + A, direction.Y + B, ProjectileID.MartianTurretBolt, npc.damage, 1, Main.myPlayer, 0, 0);
                    }
                }
        ...
        {
            if (!NPC.AnyNPCs(ModContent.NPCType<Photon>()) && !NPC.AnyNPCs(ModContent.NPCType<Quasar>()))
