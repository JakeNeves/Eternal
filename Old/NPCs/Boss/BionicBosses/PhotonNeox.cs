...
{
    public class PhotonNeox : ModNPC
    {
        ...
        {
            DisplayName.SetDefault("EXE-3068 Photon-N30X");
        ...
        public override void HitEffect(int hitDirection, double damage)
        {
            if (npc.life <= 0)
            {
                if (!NPC.AnyNPCs(ModContent.NPCType<ProtonNeox>()) && !NPC.AnyNPCs(ModContent.NPCType<QuasarNeox>()))
                    CombatText.NewText(npc.Hitbox, Color.Red, "SYSTEM FAILIURES DETECTED, CONTACTING MACHINE EXR-2350...", dramatic: true);
            }
            else
            {
