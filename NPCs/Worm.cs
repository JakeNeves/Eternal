...
//Literally every mod ever has lots and lots of fucking worms, thye need to either reduce or remove modded worms lol (this is satire btw...)
    ...
    public abstract class Worm : ModNPC
    {
		...
		public override void AI()
		{
			...
			if (Main.netMode != NetmodeID.MultiplayerClient)
			{
				if (!tail && npc.ai[0] == 0f)
				{
					if (head)
					{
						...
						npc.ai[2] = (float)Main.rand.Next(minLen, maxLen + 1);
			...
			else
			{
				...
				{
					if (!flying && npc.behindTiles && npc.soundDelay == 0)
					{
