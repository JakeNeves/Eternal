...
{
    public class QuasarNeox : ModNPC
    {
        ...
        {
            DisplayName.SetDefault("EXE-3090 Quasar-N30X");
        ...
        public override void HitEffect(int hitDirection, double damage)
        {
            if (npc.life <= 0)
            {
                if (!NPC.AnyNPCs(ModContent.NPCType<PhotonNeox>()) && !NPC.AnyNPCs(ModContent.NPCType<ProtonNeox>()))
                    CombatText.NewText(npc.Hitbox, Color.Green, "SYSTEM FAILIURES DETECTED, CONTACTING MACHINE EXR-2350...", dramatic: true);
            }
            else
            {
