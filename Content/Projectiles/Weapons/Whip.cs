...
namespace Eternal.Content.Projectiles.Weapons
{
	/// <summary>
	/// Whip things and stuff
	/// </summary>
    public abstract class Whip : ModProjectile
    {
		...
		public void FillWhipControlPoints(Projectile proj, List<Vector2> controlPoints)
		{
			...
			float num2 = 0.5f;
			float num3 = 1f + num2;
			float num4 = (float)Math.PI * 10f * (1f - num * num3) * -proj.spriteDirection / segments;
			float num5 = num * num3;
			float num6 = 0f;
			if (num5 > 1f)
			{
			...
			num7 = ContentSamples.ItemsByType[heldItem.type].useAnimation * 2 * num * player.whipRangeMultiplier;
			...
			float num11 = 0f + (float)Math.PI / 2f + (float)Math.PI / 2f * proj.spriteDirection;
		...
		public virtual void ExtraWhipBehavior()
		{
		}
		...
		public override void CutTiles()
		{
			Projectile.WhipPointsForCollision.Clear();
			FillWhipControlPoints(Projectile, Projectile.WhipPointsForCollision);
			Vector2 zero = new(Projectile.width * Projectile.scale / 2f, 0f);
			for (int flag = 0; flag < Projectile.WhipPointsForCollision.Count; flag++)
			{
				DelegateMethods.tilecut_0 = TileCuttingContext.AttackProjectile;
				Utils.PlotTileLine(Projectile.WhipPointsForCollision[flag] - zero, Projectile.WhipPointsForCollision[flag] + zero, Projectile.height * Projectile.scale, DelegateMethods.CutTiles);
			}
		}
