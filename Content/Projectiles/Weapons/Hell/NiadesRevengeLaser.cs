...
{
    public class NiadesRevengeLaser : ModProjectile
    {
        private const float PiBeamDivisor = MathHelper.Pi / NiadesRevengeHoldout.NumBeams;
        ...
        private const float MaxBeamLength = 3600f;
        ...
        private const float BeamColorHue = 2.3f;
        private const float BeamHueVariance = 1.59f;
        private const float BeamColorSaturation = 1.67f;
        ...
        public override void AI()
        {
            ...
            if (Projectile.type != ModContent.ProjectileType<NiadesRevengeLaser>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<NiadesRevengeHoldout>())
            {
                Projectile.Kill();
                return;
            }
            ...
            float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / NiadesRevengeHoldout.MaxCharge, 0f, 1f);

            Projectile.damage = (int)(hostPrism.damage * GetDamageMultiplier(chargeRatio));

            Projectile.friendly = hostPrism.ai[0] > NiadesRevengeHoldout.DamageStart;

            float beamIdOffset = BeamID - NiadesRevengeHoldout.NumBeams / 2f + 0.5f;
            float beamSpread;
            float spinRate;
            ...
            float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * NiadesRevengeHoldout.NumBeams) * MathHelper.TwoPi;
        ...
        {
            float hue = (BeamID / NiadesRevengeHoldout.NumBeams) % BeamHueVariance + BeamColorHue;
        ...
        private void ProduceBeamDust(Color beamColor)
        {
            ...
            Dust dust = Dust.NewDustDirect(endPosition, 0, 0, DustID.Shadowflame, velocity.X, velocity.Y, 0, beamColor, scale);
