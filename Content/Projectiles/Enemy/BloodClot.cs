    ...
    public class BloodClot : ModProjectile
    {
        ...
        public override void AI()
        {
            if (Main.rand.NextBool(5))
            {
                for (int i = 0; i < 5; i++)
                {
                    Dust dust = Dust.NewDustPerfect(Projectile.position, DustID.Blood);
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            if (Main.rand.NextBool(2))
                Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.Blood, Projectile.oldVelocity.X * 0.5f, Projectile.oldVelocity.Y * 0.5f);
        }
