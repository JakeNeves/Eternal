...
namespace Eternal.Invasion
{
    class ProgressBar
    {
        ...
        {
            //ripped from another mod lol, credit to them...
            float alpha = 0.5f;
            Texture2D backGround1 = Main.colorBarTexture;
            Texture2D progressColor = Main.colorBarTexture;
            Texture2D DCIcon = Eternal.instance.GetTexture("Invasion/DCIcon");
            ...
            float cleared = DroxClanWorld.DCPoints / 200f;
            ...
            spriteBatch.Draw(DCIcon, icon, Color.White);
            Utils.DrawBorderString(spriteBatch, "Drox Clan", new Vector2(barrierBackground.X + barrierBackground.Width * 0.5f, barrierBackground.Y - internalOffset - descSize.Y * 0.5f), Color.White, 0.8f, 0.3f, 0.4f);
        }
    }
}
